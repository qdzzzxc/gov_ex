...
- Регистрозависимость; в тексте XML обязательно экранировать `<` и `&`, а в атрибутах — ещё и тот тип кавычки, которым атрибут ограничен. Символ `>` обычно можно писать напрямую, кроме специальных случаев вроде `]]>`.
